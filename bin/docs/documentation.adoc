...
class VeterinairesController {
    +getVeterinaires(double latitude, double longitude): String
    -Veterinary
}
...
[plantuml]
----
class ParcsController {
    +getParc(double latitude, double longitude): String
}
...
ParcsController --> RestTemplate: restTemplate
RestTemplate --> API: Overpass API
----
